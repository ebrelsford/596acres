...
class Migration(DataMigration):
    sandy_lot_layer_names = ('sandy_distribution_sites','sandy_dropoff_sites',)

    def forwards(self, orm):
        "Add sandy lot layers"
        for layer_name in self.sandy_lot_layer_names:
            layer = orm.LotLayer(name=layer_name)
            layer.save()

    def backwards(self, orm):
        "Remove sandy lot layers"
        orm.LotLayer.objects.filter(name__in=self.sandy_lot_layer_names).delete()
