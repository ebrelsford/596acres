...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Set should_be_imported=False for old reviews"

        # select lots with more than one review
        for lot in orm.Lot.objects.exclude(review=None).annotate(review_count=models.Count('review')).filter(review_count__gt=1):
            # select their reviews except the most recent
            for review in lot.review_set.all().order_by('-added')[1:]:
                review.should_be_imported=False
                review.save()

    def backwards(self, orm):
        "Undo marking should_be_imported"
        orm.Review.objects.all().update(should_be_imported=None)
