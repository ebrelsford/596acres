...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Cache accessible data from reviews"
        for lot in orm.Lot.objects.all():
            try:
                review = orm.Review.objects.filter(lot=lot).order_by('-added')[0]
                lot.accessible = review.accessible
                lot.save()
            except:
                continue
    ...
    def backwards(self, orm):
        "undo accessible import"
        orm.Lot.objects.all().update(accessible=True)
