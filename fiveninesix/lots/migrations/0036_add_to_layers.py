...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Create current LotLayers, add Lots to them appropriately.
        """
        layer = {
            'garden_lots': Q(
        ...
        for name, filters in layer.items():
            layer = orm.LotLayer(name=name)
            layer.save()
            layer.lots = orm.Lot.objects.filter(filters)

    def backwards(self, orm):
        """
        Remove all LotLayers.
        """
        orm.LotLayer.objects.all().delete()
